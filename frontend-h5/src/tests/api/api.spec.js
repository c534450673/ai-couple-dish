...
const cases = [
  ...
  ['coupleApi.getCodeInfo', 'get', () => coupleApi.getCodeInfo(), ['/couple/codeInfo'], { coupleCode: 'A1B2C3D4', remainingSeconds: 604800 }],
  ...
  ['feedApi.getTodayFeedStatus', 'get', () => feedApi.getTodayFeedStatus(), ['/feed/today'], { status: 'pending' }],
  ['feedApi.sendFeed', 'post', () => feedApi.sendFeed({ dishName: '菜品' }), ['/feed/send', { dishName: '菜品' }], { sent: true }],
  ['feedApi.getReceivedFeeds', 'get', () => feedApi.getReceivedFeeds(), ['/feed/received'], []],
  ['feedApi.getSentFeeds', 'get', () => feedApi.getSentFeeds(), ['/feed/sent'], []],
