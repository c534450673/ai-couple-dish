...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("roles_json", sa.Text(), nullable=False, server_default="[\"admin\"]"),
...
def downgrade() -> None:
    LOGGER.warning("event=admin_analytics_migration_downgrade_started operation=downgrade result=started")
    for table in ("analytics_daily", "analytics_hourly", "analytics_event", "admin_audit_log", "admin_session", "admin_user"):
        op.drop_table(table)
    LOGGER.warning("event=admin_analytics_migration_downgrade_completed operation=downgrade result=success")
