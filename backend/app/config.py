...
class Settings(BaseSettings):
    ...
    api_prefix: str = "/api"
    # 仅用于本地/测试环境闭环验证，生产环境永不回显验证码。
    expose_dev_verification_code: bool = Field(False, alias="EXPOSE_DEV_VERIFICATION_CODE")
    ...
    @model_validator(mode="after")
    def validate_dev_verification_code(self) -> "Settings":
        if self.app_env in {"prod", "staging"} and self.expose_dev_verification_code:
            raise ValueError("生产/预发布环境不得回显验证码")
        return self
