package com.aicoupledish.common.utils;

import java.time.LocalDate;
import java.time.chrono.ChronoLocalDate;
import java.time.chrono.Chronology;
import java.time.chrono.IsoChronology;
import java.time.format.DateTimeFormatter;
import java.time.temporal.ChronoUnit;
import java.util.Arrays;
import java.util.List;

/**
 * 农历日历工具类
 * 支持农历与阳历的相互转换
 */
public class LunarCalendarUtils {

    // 农历年份信息
    // 每个数值的含义：
    // 低5位：闰月月份（0表示无闰月）
    // 6-20位：各月大小（1大月30天，0小月29天）
    private static final long[] LUNAR_INFO = {
        0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2,
        0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977,
        0x04970, 0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970,
        0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950,
        0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2, 0x0a950, 0x0b557,
        0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5b0, 0x14573, 0x052b0, 0x0a9a8, 0x0e950, 0x06aa0,
        0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0,
        0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b6a0, 0x195a6,
        0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570,
        0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x05ac0, 0x0ab60, 0x096d5, 0x092e0,
        0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5,
        0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930,
        0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260, 0x0ea65, 0x0d530,
        0x05aa0, 0x076a3, 0x096d0, 0x04afb, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45,
        0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0,
        0x14b63, 0x09370, 0x049f8, 0x04970, 0x064b0, 0x168a6, 0x0ea50, 0x06b20, 0x1a6c4, 0x0aae0,
        0x0a2e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x05d55, 0x056a0, 0x0a6d0, 0x055d4,
        0x052d0, 0x0a9b8, 0x0a950, 0x0b4a0, 0x0b6a6, 0x0ad50, 0x055a0, 0x0aba4, 0x0a5b0, 0x052b0,
        0x0b273, 0x06930, 0x07337, 0x06aa0, 0x0ad50, 0x14b55, 0x04b60, 0x0a570, 0x054e4, 0x0d160,
        0x0e968, 0x0d520, 0x0daa0, 0x16aa6, 0x056d0, 0x04ae0, 0x0a9d4, 0x0a2d0, 0x0d150, 0x0f252,
        0x0d520
    };

    // 农历月份名称
    private static final List<String> LUNAR_MONTH_NAMES = Arrays.asList(
        "正月", "二月", "三月", "四月", "五月", "六月",
        "七月", "八月", "九月", "十月", "冬月", "腊月"
    );

    // 农历日期名称
    private static final List<String> LUNAR_DAY_NAMES = Arrays.asList(
        "初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十",
        "十一", "十二", "十三", "十四", "十五", "十六", "十七", "十八", "十九", "二十",
        "廿一", "廿二", "廿三", "廿四", "廿五", "廿六", "廿七", "廿八", "廿九", "三十"
    );

    // 天干
    private static final List<String> TIAN_GAN = Arrays.asList(
        "甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸"
    );

    // 地支
    private static final List<String> DI_ZHI = Arrays.asList(
        "子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"
    );

    // 生肖
    private static final List<String> SHENG_XIAO = Arrays.asList(
        "鼠", "牛", "虎", "兔", "龙", "蛇", "马", "羊", "猴", "鸡", "狗", "猪"
    );

    /**
     * 获取农历年份的总天数
     */
    private static int getLunarYearDays(int year) {
        int sum = 348;
        for (int i = 0x8000; i > 0x8; i >>= 1) {
            sum += ((LUNAR_INFO[year - 1900] & i) != 0) ? 1 : 0;
        }
        return sum + getLeapMonthDays(year);
    }

    /**
     * 获取闰月的天数
     */
    private static int getLeapMonthDays(int year) {
        if (getLeapMonth(year) == 0) {
            return 0;
        }
        return ((LUNAR_INFO[year - 1900] & 0x10000) != 0) ? 30 : 29;
    }

    /**
     * 获取闰月月份（0表示无闰月）
     */
    private static int getLeapMonth(int year) {
        return (int) (LUNAR_INFO[year - 1900] & 0xf);
    }

    /**
     * 获取农历月份的天数
     */
    private static int getLunarMonthDays(int year, int month) {
        return ((LUNAR_INFO[year - 1900] & (0x10000 >> month)) != 0) ? 30 : 29;
    }

    /**
     * 阳历转农历
     * @param solarDate 阳历日期
     * @return 农历日期数组 [年, 月, 日, 是否闰月]
     */
    public static int[] solarToLunar(LocalDate solarDate) {
        int year = solarDate.getYear();
        int month = solarDate.getMonthValue();
        int day = solarDate.getDayOfMonth();

        // 计算与1900年1月31日的天数差
        LocalDate baseDate = LocalDate.of(1900, 1, 31);
        long offset = ChronoUnit.DAYS.between(baseDate, solarDate);

        // 计算农历年
        int lunarYear = 1900;
        int yearDays;
        while (lunarYear < 2100 && offset > 0) {
            yearDays = getLunarYearDays(lunarYear);
            if (offset < yearDays) {
                break;
            }
            offset -= yearDays;
            lunarYear++;
        }

        // 计算农历月
        int lunarMonth = 1;
        int leapMonth = getLeapMonth(lunarYear);
        boolean isLeap = false;
        int monthDays;

        while (lunarMonth < 13 && offset > 0) {
            if (leapMonth > 0 && lunarMonth == (leapMonth + 1) && !isLeap) {
                --lunarMonth;
                isLeap = true;
                monthDays = getLeapMonthDays(lunarYear);
            } else {
                monthDays = getLunarMonthDays(lunarYear, lunarMonth);
            }

            if (offset < monthDays) {
                break;
            }
            offset -= monthDays;

            if (isLeap && lunarMonth == (leapMonth + 1)) {
                isLeap = false;
            }
            if (!isLeap) {
                lunarMonth++;
            }
        }

        // 农历日
        int lunarDay = (int) offset + 1;

        return new int[]{lunarYear, lunarMonth, lunarDay, isLeap ? 1 : 0};
    }

    /**
     * 农历转阳历
     * @param lunarYear 农历年
     * @param lunarMonth 农历月
     * @param lunarDay 农历日
     * @param isLeap 是否闰月
     * @return 阳历日期
     */
    public static LocalDate lunarToSolar(int lunarYear, int lunarMonth, int lunarDay, boolean isLeap) {
        // 计算从1900年1月31日（农历1900年正月初一）开始的天数
        LocalDate solarDate = LocalDate.of(1900, 1, 31);
        long offset = 0;

        // 累加年份天数
        for (int y = 1900; y < lunarYear; y++) {
            offset += getLunarYearDays(y);
        }

        // 累加月份天数
        int leapMonth = getLeapMonth(lunarYear);
        for (int m = 1; m < lunarMonth; m++) {
            offset += getLunarMonthDays(lunarYear, m);
            if (m == leapMonth) {
                offset += getLeapMonthDays(lunarYear);
            }
        }

        // 处理闰月
        if (isLeap && lunarMonth == leapMonth) {
            offset += getLunarMonthDays(lunarYear, lunarMonth);
        }

        // 累加日期天数
        offset += lunarDay - 1;

        return solarDate.plusDays(offset);
    }

    /**
     * 获取农历日期的中文名称
     */
    public static String getLunarDateName(int year, int month, int day, boolean isLeap) {
        StringBuilder sb = new StringBuilder();
        sb.append(getLunarYearName(year)).append("年");
        if (isLeap) {
            sb.append("闰");
        }
        sb.append(LUNAR_MONTH_NAMES.get(month - 1));
        sb.append(LUNAR_DAY_NAMES.get(day - 1));
        return sb.toString();
    }

    /**
     * 获取农历年份名称（干支年）
     */
    public static String getLunarYearName(int year) {
        int ganIndex = (year - 4) % 10;
        int zhiIndex = (year - 4) % 12;
        return TIAN_GAN.get(ganIndex) + DI_ZHI.get(zhiIndex);
    }

    /**
     * 获取生肖
     */
    public static String getShengXiao(int year) {
        return SHENG_XIAO.get((year - 4) % 12);
    }

    /**
     * 获取下一次农历日期对应的阳历日期
     * 用于农历生日/纪念日计算
     * @param lunarMonth 农历月
     * @param lunarDay 农历日
     * @return 下一次对应的阳历日期
     */
    public static LocalDate getNextLunarDate(int lunarMonth, int lunarDay) {
        LocalDate today = LocalDate.now();
        int currentYear = today.getYear();

        // 先尝试今年
        LocalDate solarDate = lunarToSolar(currentYear, lunarMonth, lunarDay, false);
        if (solarDate.isAfter(today)) {
            return solarDate;
        }

        // 再尝试明年
        return lunarToSolar(currentYear + 1, lunarMonth, lunarDay, false);
    }

    /**
     * 判断是否是农历节日
     */
    public static String getLunarHoliday(int lunarMonth, int lunarDay) {
        // 农历节日
        if (lunarMonth == 1 && lunarDay == 1) return "春节";
        if (lunarMonth == 1 && lunarDay == 15) return "元宵节";
        if (lunarMonth == 5 && lunarDay == 5) return "端午节";
        if (lunarMonth == 7 && lunarDay == 7) return "七夕节";
        if (lunarMonth == 7 && lunarDay == 15) return "中元节";
        if (lunarMonth == 8 && lunarDay == 15) return "中秋节";
        if (lunarMonth == 9 && lunarDay == 9) return "重阳节";
        if (lunarMonth == 12 && lunarDay == 30) return "除夕";
        if (lunarMonth == 12 && lunarDay == 8) return "腊八节";
        return null;
    }
}
