...
const syncBoundCouple = async () => {
  if (completed.value) return
  const result = await userStore.getCoupleInfo()
  if (result?.status !== 'success' || !result.data) return
  completed.value = true
  logUiEvent('couple.bind', {
    module: 'couple', operation: 'poll_bound_status', result: 'success', durationMs: 0, userId: maskedUserId()
  })
  router.replace(safeRedirect(route.query.redirect))
}
...
onMounted(() => {
  loadCode()
  couplePollTimer = window.setInterval(syncBoundCouple, 3000)
})

onUnmounted(() => {
  if (couplePollTimer !== null) window.clearInterval(couplePollTimer)
  couplePollTimer = null
})
</script>
